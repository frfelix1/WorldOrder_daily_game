...
function performanceLabel(score: number): { label: string; color: string; medal: string; glow: string } {
  if (score === 1000) return { label: 'Perfect',         color: '#e8c547', medal: '🏆', glow: 'rgba(232,197,71,0.4)' };
  if (score >= 800)  return { label: 'Excellent',       color: '#00c4e8', medal: '🥇', glow: 'rgba(0,196,232,0.35)' };
  if (score >= 600)  return { label: 'Great',           color: '#00e896', medal: '🥈', glow: 'rgba(0,232,150,0.35)' };
  if (score >= 400)  return { label: 'Good',            color: '#a3e635', medal: '🥉', glow: 'rgba(163,230,53,0.3)' };
